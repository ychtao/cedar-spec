...
theorem chainComp_head_error {headExpr : Expr} {astTail : List (Cst.RelOp × Expr)}
    ...
    evaluate (Cst.chainComp headExpr astTail) req es = .error e := by
  match astTail with
  | [] => simpa [Cst.chainComp] using h
  | [(op, x)] =>
    simp only [Cst.chainComp]
    cases op <;> simp [Cst.constructExprRel, evaluate, h, bind, Except.bind]
  | (op, x) :: hd' :: tl' =>
    simp only [Cst.chainComp]
    cases op <;> simp [Cst.constructExprRel, evaluate, h, bind, Except.bind, Result.as]
...
/-- Agreement between the translator's `chainComp` (an `&&`-tree of comparisons
    ...
    same `.ok` results), the AST expression `Cst.chainComp headExpr astTail` and
...
theorem chainComp_chainCompEval_agrees (req : Request) (es : Entities) :
    ...
    List.Forall₂ (fun cp ap => cp.1 = ap.1 ∧
      ...
      cstTail astTail →
    ∀ v, evaluate (Cst.chainComp headExpr astTail) req es = .ok v ↔
  ...
  | nil =>
    ...
    cases hrel
    simp [Cst.chainComp, Cst.chainCompEval, hhead]
  | cons cp cps ih =>
    intro astTail headExpr headVal hhead hrel v
    cases hrel with
    | cons hhd htl =>
      ...
      | nil =>
        cases htl
        simp only [Cst.chainComp, Cst.chainCompEval]
            ...
            | ok cur =>
              ...
              | ok bb =>
                ...
                | true =>
                  ...
                  cases hB : evaluate (Cst.chainComp ax (ap' :: aps')) req es with
                  | error e =>
...
theorem chainComp_chainCompEval_eq (req : Request) (es : Entities) :
    ...
    List.Forall₂ (fun cp ap => cp.1 = ap.1 ∧ evaluate ap.2 req es = cp.2.evaluate req es)
      cstTail astTail →
    evaluate (Cst.chainComp headExpr astTail) req es = Cst.chainCompEval headVal cstTail req es := by
  intro cstTail
  induction cstTail with
  | nil =>
